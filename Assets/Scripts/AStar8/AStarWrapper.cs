...
namespace AStar
{
    ...
    {
        public static AStarWrapper Ins = new AStarWrapper();
        /// <summary>
        /// 根据开始点和结束点，取得较短路径
        /// </summary>
        ...
        private List<Point> GetSurroundPoints(Point point, Point[,] map, int mapWidth, int mapHeight)
        {
            ...
            if (lu != null && lu.IsWall == false)
            {
                list.Add(lu);
            }
            if (ru != null && ru.IsWall == false)
            {
                list.Add(ru);
            }
            if (ld != null && ld.IsWall == false)
            {
                list.Add(ld);
            }
            if (rd != null && rd.IsWall == false)
            {
                list.Add(rd);
            }
